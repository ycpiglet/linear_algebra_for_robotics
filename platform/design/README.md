...
| `trust-bootstrap` | GitHub App authority, CODEOWNERS, review ruleset, provenance required 전환 |
...
```text
PUB-001 → PUB-002 → PUB-016 → PUB-017
PUB-017 → PUB-003 (decision) → PUB-004 → PUB-005 → PUB-007 → PUB-008
PUB-017 → PUB-006
PUB-006 + PUB-007 + PUB-008 → PUB-009
PUB-009 ───────────┬─ PUB-010
...
- `PUB-002`의 read-only 안전망, `PUB-016`의 provenance gate, `PUB-017`의 역할 신뢰 결박 전에는
  외부 rename, artifact inventory, foundation migration을 시작하지 않는다.
...
PUB-017 전환 뒤 agent 작업은 저장소 전용 GitHub App identity로만 게시한다. App은 각 PR의
exact head에 merge commit 방식 auto-merge를 등록하고 `source-contract`, `publication-build`,
`trusted-provenance`가 모두 성공할 때까지 기다린다. 일반 경로는 사람 승인 없이 병합한다.

다음 trust-control 경로를 바꾸는 PR만 `.github/CODEOWNERS`의 `@ycpiglet` 승인이 필요하다.

- `.github/CODEOWNERS`, `.github/workflows/**`와 다른 CODEOWNERS 후보
- provenance verifier와 privileged editorial controller·runtime·schema
- 위 경계를 추가로 확장하는 canonical 보호 경로 목록

steady-state main ruleset은 전역 승인 수 0, CODEOWNER 승인 필수, stale 승인 폐기,
last-push 전역 승인 비활성, bypass 없음으로 유지한다. last-push 승인은 전역 규칙이라 켜지 않는다.
각 App PR은 target repository ID `1300261697` 안에서 `agent/pr-<40자리 head SHA>` ref를 한 번만
생성한다. fork head는 허용하지 않는다. 별도 `agent/pr-*` ruleset은
creation만 허용하고 update·delete를 bypass 없이 막으므로 승인 뒤 head를 바꿀 수 없다. base-side
`trusted-provenance`도 보호 경로의 live head와 `@ycpiglet`의 APPROVED review
`commit_id`를 일치시켜, 최종 diff가 같은 새 commit처럼 GitHub의 stale-review 규칙만으로 잡히지
않는 head 교체를 이중 검사한다. GitHub Actions integration ID는 모든 workflow가 공유하므로 이
status만을 단독 head lock으로 간주하지 않는다. App PR merge queue는 직렬화하고, strict base가
앞서가면 GitHub의 update-branch를 쓰지 않는다. 기존 auto-merge를 끄고 PR을 닫은 뒤 최신 main을 단일 parent로 한
새 App commit/ref/PR로 같은 변경을 재생성한다. 보호 변경은 새 알림·승인을 요구하고 일반 변경은
사람 없이 다시 진행한다. 일반 PR에는 owner 승인 조건을 적용하지 않는다. allimbot 알림은 owner가
고정한 App bot author와 target same-repository immutable ref를 live 재확인한 보호 PR에만 보내며,
보호 이유·head SHA·GitHub 링크만 포함한다. allimbot 자체는 review·merge·admin authority를 갖지 않는다.
`--admin` 병합이나 required check 위조는 금지한다.
base-side controller는 owner가 설치 IAT의 `viewer.login`으로 확정한 non-secret
`AGENT_APP_BOT_LOGIN` 변수와 live PR author도 일치시킨다. label과 branch 이름만으로 agent identity를
인증하지 않는다.
review 이벤트의 PR merge-ref workflow는 권한 없는 no-op 신호만 보낸다. 실제 판정은
default-branch `workflow_run` controller가 live API와 새 test-merge를 재검증하므로 신호 run의
conclusion이나 payload 자체를 승인 증거로 사용하지 않는다.
보호 경로 변경은 supervisor/editor가 직접 게시하지 않고 authenticated App/broker의 immutable
same-repository PR로 재게시한다. steady App의 Workflows 권한은 없으므로 향후 workflow 변경은
Actions-off 외부 exact-tree broker의 일회성 권한 창을 다시 사용한다. legacy `editorial/batch`는 기존
default-branch digest 계약의 좁은 예외이며 immutable App PR namespace로 분류하지 않는다.

CODEOWNERS와 exact-head review gate가 아직 base branch에 없는 최초 transition PR만 임시 전역
승인 1건과 last-push 승인으로 보호한다. 그 PR을 별도 `@ycpiglet`이 승인·병합한 뒤
`trusted-provenance`를 required로 묶으면서 위 steady-state 규칙으로 전환하고 canary를 실행한다.
최초 `agent/pub-017-trust-transition` ref도 steady `agent/pr-*`와 같은 update·deletion ruleset에
포함하되 creation 제한은 두지 않아, 한 번 생성한 bootstrap head를 App이나 owner가 바꿀 수 없게 한다.
