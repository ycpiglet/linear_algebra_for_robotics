...
```text
PUB-001 → PUB-002 → PUB-016 ─┬→ PUB-017 (decision paused; 별도 재개 결정 필요)
                              └→ PUB-018 (temporary fallback)
PUB-018 ─┬→ PUB-003 (decision) → PUB-004 → PUB-005
         ├→ PUB-006
         └→ PUB-007 → PUB-008
...
PUB-008 + PUB-017 → PUB-019 (deferred CI activation)
```

- `PUB-017`의 GitHub App·Windows broker trust transition은 2026-07-23 owner 결정으로 보류했다.
  PR #25와 증거는 보존하지만 기존 transition ref를 재사용하거나 r6식 live recovery를 반복하지 않는다.
  재개하려면 현재 외부 상태를 다시 읽고, 단순 patch가 아닌 새 설계 감사와 명시적 재개 결정부터 한다.
- 임시 운영 모드는 `PUB-002` read-only 안전망과 `PUB-016`의 기존
...
- 일반 변경은 `agent/*` branch, 실제 `Actor: agent` commit trailer, `actor:agent` PR label을 사용하고
  기존 `source-contract`·`publication-build`가 exact head에서 성공해야 한다.
- 현재 owner 계정 세션을 agent가 보조하는 방식은 독립된 supervisor 승인을 기술적으로 강제하지
  못한다. 사용자가 승인한 범위의 source PR은 reviewed exact head를 merge commit으로 병합하되,
  이 절차를 별도 GitHub authority의 승인으로 기록하거나 홍보하지 않는다.
- repository rename·custom domain·ruleset·workflow·App permission·secret·deploy credential처럼
  외부 상태나 통제면을 바꾸는 작업은 별도 사용자 확인 없이는 실행하지 않는다. aliimbot 알림은
  확인 요청 수단일 뿐 credential 보관자나 GitHub approval 주체로 취급하지 않는다.
