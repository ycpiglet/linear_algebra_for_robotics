...
권위가 충돌하면 다음 문서가 이 백로그보다 우선한다.

1. `AUTHORING_MANUAL.md`: 사용자 요구, 콘텐츠·근거·완료 기준
2. `design-system.qmd`: 승인된 시각·접근성 계약과 실제 렌더 표본
3. `assets/README.md`: 자산 책임과 변경 원칙
4. `EDITING_SYSTEM_PLAN.md`: 교정 이슈·이벤트·규칙 승격 운영
...
- `blocked`: blocker와 재검토 조건을 기록한다.
- `rolled_back`: revert 근거와 후속 작업 ID를 evidence에 남긴다.
...
6. 같은 `exclusive_locks` 값을 가진 `in_progress`/`in_review` 항목은 동시에 존재할 수 없다.
7. 병렬 PR의 경로가 겹치면 후속 PR은 최신 main에서 다시 적용하고 전체 required gate를 재실행한다.
8. PR은 merge commit으로 병합하고 한 PR에는 공통 계약 하나 또는 migration batch 하나만 둔다.
...
계획 기준선은 `main`의 `5a79cae`이며, 계획 당시 열린 PR은 `#14` 한 건이다. 이 PR은
`README.md`와 `remote-access/`만 바꾸므로 `PUB-001`과 직접 충돌하지 않는다. 저장소 cutover
전에는 모든 열린 PR을 병합·종결하거나 carry-over 결정을 기록해야 한다. 계획 당시 main에는
branch protection/ruleset이 없고 미처리 editorial 제안과 열린 `editorial/batch` PR은 없다.
`PUB-002`에서 required read-only check와 명시적인 editorial freeze switch를 먼저 마련하고,
cutover 직전에 이 외부 상태를 다시 확인한다.
