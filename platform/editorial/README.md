...
key를 교체할 때도 먼저 위의 freeze·drain 절차를 수행한다. 새 write deploy key를 추가하고
`EDITORIAL_BATCH_SSH_KEY` secret을 같은 점검 창에서 교체한다. 단, pending 이슈가 0건이면
성공한 `repository_dispatch`도 사전 필터까지만 실행되므로 새 key를 검증한 것이 아니다.
실제 또는 통제된 test proposal 1건으로 run이 `Checkout isolated batch push credential`과
`Push batch branch`에 도달했는지 확인하고, 원격 batch ref가 run의 `head_sha`와 같은지 및
새 deploy key의 `last_used`가 교체 뒤 시각으로 갱신됐는지 확인한 뒤에만 이전 key를 제거한다.
이 조건을 충족하지 못하면 이전 key를 제거하지 말고 동결 상태에서 원인을 해결한다. 개인 PAT나
계정 SSH key를 이 용도로 재사용하지 않는다.
...
rollback PR로 workflow를 되돌린다. 이전 source push→batch PR→read-only quality dispatch 경로가
복귀한 것을 확인한 뒤 전용 deploy key·secret과 비활성 ruleset을 철회하고 freeze를 해제한다.
ruleset을 둔 채 workflow만 revert하면 다음 `GITHUB_TOKEN` batch push가 거부되므로 rollback
완료로 판정하지 않는다.
