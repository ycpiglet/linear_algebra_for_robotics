...
difficulty: 3
importance: 5
importance_note: "자코비안은 순기구학을 현재 자세에서 국소 선형화해 관절의 작은 변화·속도와 작업공간의 작은 변화·속도를 연결하며, 역기구학·힘/임피던스 제어·특이점 분석·보정·비선형 추정에서 반복 사용한다."
...
review:
  prompts:
    ...
    - "2R 평면팔의 순기구학에서 자코비안을 중간 단계를 생략하지 않고 유도할 수 있는가?"
...
로봇의 순기구학(*forward kinematics*, FK; 정기구학이라고도 함)을 $x=f(q)$라 하자. 현재 관절각 $q$ 근처에서는 복잡한 비선형 관계도
...
지구는 둥글지만 동네 지도는 평면처럼 쓸 수 있다. 충분히 작은 영역만 확대하면 곡면이 접평면처럼 보이기 때문이다. 로봇의 순기구학도 전체적으로는 사인(sin)과 코사인(cos)이 얽힌 비선형 함수지만, **현재 자세의 아주 가까운 곳**에서는 행렬 하나로 근사할 수 있다.
...
- **순기구학**(*forward kinematics*, FK): 관절 좌표 $q$에서 말단 위치·자세 $x$를 계산하는 함수 $x=f(q)$
...
$l=(0.4,0.3)$ m, $q=(30^\circ,60^\circ)$, $\Delta q=(10^{-4},-2\times10^{-4})$ rad에서 직접 순기구학을 두 번 계산하면
...
**병목:** 일반 직렬 로봇에서는 순기구학과 $J(q)$ 갱신이 행렬–벡터 곱보다 비싸므로, 같은 자세에서 중복 계산하지 않되 오래된 캐시를 재사용하지 않는다.\
